...
async function readDomainPack() { return null; }
...
TAALREGELS — verplicht in al je suggesties en herschrijven:
- VERBODEN: em-dashes (— en –). Altijd vervangen door komma, punt of herschrijven.
- Geen onnodige hoofdletters mid-sentence ("de Dienst", "het Product", "uw Organisatie").
- Correct Nederlands (NLP). Geen Engelse constructies.`;
...
TAALREGELS — verplicht in je revision_prompt:
- VERBODEN: em-dashes (— en –). Vervang door komma, punt of herschrijf.
- Geen onnodige hoofdletters mid-sentence ("de Dienst", "het Product", "uw Organisatie").
- Schrijf revisie-instructies in correct Nederlands (NLP). Geen Engelse constructies.
...
async function callSynthesizer({ content, reports, brief }) {
  ...
  try {
    ...
    // 'orange' i.p.v. 'red' — synthesizer-fout blokkeert publicatie niet onnodig.
    return { verdict: 'orange', revision_prompt: '', summary: 'Synthesizer tijdelijk niet beschikbaar: ' + err.message };
  }
}
